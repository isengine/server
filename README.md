...
> Мы предлагаем уже настроенные проекты для сервисов, входящих в комплект. Если вы не используете какие-то из них, **nginx** может не запуститься. Для отключения переименуйте расширение, например **api.http.conf.off**

...
Перед началом, установите и настройте Docker:
...
**2**. Перейдите в каталог среды разработки.

```shell script
cd lds
```

**3**. Удалите все файлы **.gitkeep** во всех вложенных каталогах.
...
Иногда перед запуском, вам может потребоваться скопировать файлы из какого-то каталога в рабочий каталог сервера.

Для этого можно воспользоваться следующей командой:

```shell script
cp -r -f /var/www/custom/. /var/www/public
```

Вы можете установить некоторые зависимости, которые могут быть нужны. Например:

...
Здесь указывается папка **lds**, чтобы она не попала в ваш репозиторий.
...
Во втором случае вам понадобится лишь изменить структуру папок и внести правки в файл конфигурации nginx и в файл **hosts**.

> Мы рекомендуем использовать второй вариант.

Если вы только устанавливаете **lds**, устанавливайте его в корневую папку ваших проектов. В остальном следуйте инструкции: [Быстрый старт](#быстрый-старт).
...
Сам **lds** нужно развернуть в корневой папке проектов. Каждый проект теперь должен лежать в папке, параллельной папке **lds**.

> Под **lds** здесь понимается не только сама папка, но и служебные файлы **.env**, **docker-compose.yml**, **Dockerfile**.

Если **lds** уже был развернут в папке каждого проекта, вы можете перенести его вверх из одного проекта, а во всех остальных - удалить.
...
Здесь **DOMAIN_1** - имя вашего проекта, нужно чтобы оно совпадало с папкой, в которой он лежит и имело вид типа:
...
Адрес будет тот же самый: **127.0.0.1**, а вот имя хоста должно соответствовать именам папок ваших проектов.
